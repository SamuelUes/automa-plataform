...
import { adminClient, createExecution, invokeN8n, isUuid, verifyOrganization } from "../_shared/integration.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!conversationId) {
      const { data, error } = await client.from("conversations").insert({ organization_id: profile.organization_id, user_id: user.id, case_id: body.case_id, title: body.title || "Conversación del caso", conversation_type: "case" }).select("id").single();
    ...
    const { data: message, error: messageError } = await client.from("messages").insert({
      ...
      role: "user",
      content,
      metadata: { source: "dashboard", case_id: body.case_id },
    ...
    try {
      ...
      return Response.json({ conversation_id: conversationId, message_id: message.id, workflow_execution_id: executionId, status: "queued", n8n: result }, { status: 202, headers: corsHeaders });
    } catch (error) {
      ...
      return Response.json({ conversation_id: conversationId, message_id: message.id, workflow_execution_id: executionId, status: "failed", error: "No se pudo iniciar PE13" }, { status: 503, headers: corsHeaders });
    }
  } catch (error) {
